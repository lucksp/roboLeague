...
import { randomLetter, random3, insertItem, removeItem } from "../../helpers";
...
export default function data(state = state ? state : initialState, action) {
  switch (action.type) {
    ...
    case ActionTypes.EDIT_NAME:
      ...
      const { newName, userId, callback } = { ...action.payload };
      const newNames = newName.split(" ");

      let replaceIndex;
      newState.find((name, i) => {
      ...
      if (replaceIndex >= 0) {
        let tempState = {};
        tempState = newState[replaceIndex];

        tempState.name_first = newNames[0];
        tempState.name_last = newNames[1] ? newNames[1] : "";
        tempState.name = newNames[0] + " " + (newNames[1] ? newNames[1] : "");
